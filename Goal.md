...
| **2026-09-05** | **掃描清完就換難題**：本人明說不要再只做小修小補。同一輪先把掃描表清空（兩個小 PR），再做兩題要「算」或要「講語意」的：alignment 位移後重算（新 helper、樹裡首例）、complex fold 缺 fastmath 檢查（照 LLVM 規則） | 小題證明流程與品味，難題證明能設計；履歷要兩種都有。難題的來源仍是掃描——fastmath 掃描順手抓到 complex fold 的語意洞 |
