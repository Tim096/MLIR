...
| **2026-09-05** | **找到一個 bug 就掃全樹找同款**：#221312 的「重建 op 丟屬性」不會只有一處，用兩個 agent 平行掃 `mlir/lib` 所有 `memref`／`vector` 記憶體 op 的重建點，一次找出十一處位址不變的、四組位址會移的 | 同款問題的修法、測試、答辯都能重用，一天內從一題變六題（#221308／#221312／#221314／#221317／#221319／#221320）；分類「位址是否不變」是 reviewer 一定會問的，先答好才送 |
| **2026-09-05** | **掃描清完就換難題**：本人明說不要再只做小修小補。同一輪先把掃描表清空（兩個小 PR），再做兩題要「算」或要「講語意」的：alignment 位移後重算（新 helper、樹裡首例）、complex fold 缺 fastmath 檢查（照 LLVM 規則） | 小題證明流程與品味，難題證明能設計；履歷要兩種都有。難題的來源仍是掃描——fastmath 掃描順手抓到 complex fold 的語意洞；第三題（#221386）則是把兩週前 review 裡 maintainer 的硬體結論落實成 patch，有 issue、有規格、有討論、有實作，履歷上最完整的一條線 |
| **2026-09-05** | **平行只平行「查」，不平行「建」**：小題可以派 agent 各自出 diff ＋ 測試草稿，但套用、建置、lit、commit 一律由本人在同一個 build 目錄串行做；候選開工前先驗前提（GPU-2 查證後不是 bug，直接刷掉） | 一個 build 目錄同時只能對一個分支，agent 平行建置會互相蓋；而 agent 的 diff 只算草稿，在舊 binary 上重現、在新 binary 上跑 lit 才算驗過。這樣一個下午送了三個 PR（#221307、#221308、#221312）而不影響驗證品質 |
